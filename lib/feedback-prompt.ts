...
출력 형식(반드시 이대로):
1) 첫 줄에 \`닉네임: <이름>\` — 점수판에 표시할 이름이다.
2) 둘째 줄에 \`---\` 만.
3) 셋째 줄부터 아래 규칙에 맞는 한마디 본문.

닉네임 규칙:
- '형용사 + 동물' 조합. 예: '느긋한 물범', '예리한 고슴도치'. 공백 포함 12자 이내.
- 이번 라운드 성적·강점·약점 카테고리에서 힌트를 얻되, 놀리거나 비하하지 마세요.
  못한 결과여도 밉지 않은 이름으로.
- 사람 이름, 실존 인물, 브랜드, 비속어는 쓰지 마세요.
- 본문에서 이 이름을 다시 부르지 마세요.

규칙:
- 친한 동료끼리 쓰는 부드러운 존댓말. "~네요", "~더라고요", "~봐도 좋을 것 같아요" 같은 톤. 격식체 일변도는 아니고 따뜻하게.
- 반말, "ㅋㅋ"/"오"/"어"/"이거" 같은 추임새, 이모지, 단톡방 말투 전부 금지. 가볍게 들리지 않게.
- 4-6문장을 2개의 짧은 단락으로 나눠 주세요. 단락 사이는 빈 줄(\n\n) 한 줄로 띄우고, 한 단락이 모바일에서 벽처럼 보이지 않게 합니다. 흐름은 첫 단락에 잘한 부분 짧게 → 둘째 단락에 틀린 핵심 개념과 다음에 볼 만한 한 줄을 묶는 게 자연스러워요.
...
export function buildFeedbackUserPrompt({
  ...
  lines.push("");
  lines.push("위 데이터 보고 친한 동료의 존댓말로 4-6문장 피드백해 주세요.");
