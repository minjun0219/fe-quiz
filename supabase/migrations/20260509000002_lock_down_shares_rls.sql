...
--
-- 0001_create_shares.sql은 RLS 정책을 `to anon using(true) / with check(true)`로
-- 만들었고 20260509000001_grant_anon_on_shares.sql은 `grant select, insert ...
-- to anon`을 부여했다. 이 조합은 publishable 키만 있으면 누구나 supabase REST
-- API 직호출로:
...
-- 전환하면서 anon 경로를 닫는다. secret 키는 RLS를 우회하지만 BYPASSRLS는
-- 정책만 건너뛸 뿐 SQL GRANT를 자동 생성하진 않는다. Supabase 프로젝트의
-- "Automatically expose new tables" 옵션이 OFF면 service_role 자동 GRANT도
-- 안 붙어서 `permission denied for table shares`가 날 수 있음 →
-- service_role에 명시 GRANT 부여 필수.
--
...
-- service_role에 명시적 GRANT. RLS는 우회되지만 테이블 권한은 별도라
-- "auto-expose off" 환경에서 권한 누락으로 라우트 핸들러가 깨지는 걸 방지.
grant select, insert on public.shares to service_role;
