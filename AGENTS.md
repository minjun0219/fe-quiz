...
- **e2e는 배포본을 칩니다**: `pnpm test:e2e`는 `E2E_BASE_URL`이 없으면 로컬 서버를 띄워 안전하지만, 값을 주면 그 배포를 직접 검증해요. 프리뷰 URL을 주면 **프로덕션 D1에 쓰기가 들어갑니다**(아래 마이그레이션 항목과 같은 이유) — CI는 `[e2e-cleanup]` 마커로 잡 끝에 지워요.
