...
- **공유**: `/api/share`가 서버 재채점 후 `shares` row 생성, `/r/[slug]` 결과 페이지와 OG 이미지 제공. 공유 URL은 요청 헤더 + Vercel 운영 도메인 화이트리스트로 도출 (`NEXT_PUBLIC_SITE_URL` 폴백 제거, #36)
- **보안/남용 방지**: Supabase secret key 서버 접근, anon 직접 접근 차단, Upstash rate limit 선택 적용
- **Observability**: PostHog 서버/클라이언트 클라이언트 + Next.js `instrumentation.onRequestError`로 미처리 예외 캡처, `app/error.tsx`/`app/global-error.tsx`로 라우트 단 에러 바운더리, Vercel Analytics. 키 미설정 시 모두 no-op
...
- **Logging**: pino
- **Analytics/Error monitoring**: PostHog (`posthog-js`/`posthog-node`) + `@vercel/analytics` — 키 미설정 시 no-op
- **호스팅 가정**: Vercel
